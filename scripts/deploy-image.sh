...
# Configuration
PROJECT_NAME="tour-management-cont"
...
read -p "Enter Docker Image URI (e.g., 123456789.dkr.ecr.us-east-1.amazonaws.com/tour-management-cont:latest): " IMAGE_URI
...
if [[ "$NEED_LB" =~ ^[Yy]$ ]]; then
  ...
  for subnet in "${SUBNET_ARRAY[@]}"; do
    SUBNET_PARAM="$SUBNET_PARAM $subnet"
  done
...
if [ -n "$TARGET_GROUP_ARN" ]; then
  ...
  python3 -c "import json; data=json.load(open('ecs/service-definition-deploy.json')); data.pop('loadBalancers', None); data.pop('healthCheckGracePeriodSeconds', None); json.dump(data, open('ecs/service-definition-deploy.json', 'w'), indent=2)"
fi
