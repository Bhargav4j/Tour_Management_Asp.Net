...
public class TourRepository : ITourRepository
{
    ...
    public async Task UpdateAsync(Tour tour, CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogError(ex, "Error updating tour with ID {TourId} in database", tour.TourId);
            throw;
        }
    }
    ...
    public async Task<bool> ExistsAsync(int id, CancellationToken cancellationToken = default)
    {
        try
        {
            return await _context.Tours.AnyAsync(t => t.TourId == id && t.IsActive, cancellationToken);
        }
        catch (Exception ex)
        {
