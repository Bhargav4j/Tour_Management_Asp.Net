...
public class BookingService : IBookingService
{
    ...
    public async Task<IEnumerable<Booking>> GetUserBookingsAsync(string email, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            _logger.LogInformation("Successfully retrieved {Count} bookings for user: {Email}", bookings.Count(), email);
            return bookings;
        }
        catch (Exception ex)
        {
    ...
    public async Task<Booking> CreateBookingAsync(Booking booking, CancellationToken cancellationToken = default)
    {
        try
        {
            _logger.LogInformation("Creating new booking for tour: {TourId} and user: {Email}", booking.TourId, booking.Email);
            ...
            var userExists = await _userRepository.ExistsAsync(booking.Email, cancellationToken);
            if (!userExists)
            {
                _logger.LogWarning("User with email: {Email} not found", booking.Email);
                throw new InvalidOperationException($"User with email {booking.Email} not found");
            }
        ...
        {
            _logger.LogError(ex, "Error creating booking for tour: {TourId} and user: {Email}", booking.TourId, booking.Email);
            throw;
        }
    }
