...
public class UserService : IUserService
{
    ...
    public async Task<User?> AuthenticateAsync(string email, string password, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                _logger.LogWarning("Authentication failed for user: {Email} - User not found or inactive", email);
                return null;
            }

            if (!BCrypt.Net.BCrypt.Verify(password, user.PasswordHash))
            {
                _logger.LogWarning("Authentication failed for user: {Email} - Invalid password", email);
                return null;
            }
