...
public class TourRepository : ITourRepository
{
    ...
    public async Task<bool> ExistsAsync(int id, CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogError(ex, "Error checking if tour with ID: {TourId} exists", id);
            throw;
        }
    }
    ...
    public async Task<IEnumerable<Tour>> SearchAsync(string searchTerm, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return await _context.Tours
                .Where(t => t.IsActive &&
                    (t.TourName.ToLower().Contains(lowerSearchTerm) ||
                     t.Place.ToLower().Contains(lowerSearchTerm) ||
                     t.Locations.ToLower().Contains(lowerSearchTerm) ||
                     t.TourInfo.ToLower().Contains(lowerSearchTerm)))
                .AsNoTracking()
                .ToListAsync(cancellationToken);
        }
        catch (Exception ex)
