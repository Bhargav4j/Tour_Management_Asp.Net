using FluentAssertions;
using Microsoft.EntityFrameworkCore;
using Microsoft.Extensions.Logging;
using Moq;
using TourManagement.Domain.Entities;
using TourManagement.Infrastructure.Data;
using TourManagement.Infrastructure.Repositories;

namespace TourManagement.IntegrationTests.Repositories;

public class TourRepositoryTests : IDisposable
{
    private readonly TourManagementDbContext _context;
    private readonly TourRepository _repository;

    public TourRepositoryTests()
    {
        var options = new DbContextOptionsBuilder<TourManagementDbContext>()
            .UseInMemoryDatabase(databaseName: Guid.NewGuid().ToString())
            .Options;

        _context = new TourManagementDbContext(options);
        var mockLogger = new Mock<ILogger<TourRepository>>();
        _repository = new TourRepository(_context, mockLogger.Object);
    }

    [Fact]
    public async Task AddAsync_AddsTourToDatabase()
    {
        var tour = new Tour
        {
            TourName = "Test Tour",
            Place = "Test Place",
            Days = 5,
            Price = 100,
            Locations = "Location 1, Location 2",
            TourInfo = "Test info",
            IsActive = true,
            CreatedDate = DateTime.UtcNow,
            CreatedBy = "Test"
        };

        var result = await _repository.AddAsync(tour);

        result.Id.Should().BeGreaterThan(0);
        result.TourName.Should().Be("Test Tour");

        var savedTour = await _context.Tours.FindAsync(result.Id);
        savedTour.Should().NotBeNull();
    }

    [Fact]
    public async Task GetAllAsync_ReturnsAllActiveTours()
    {
        await _context.Tours.AddRangeAsync(
            new Tour { TourName = "Tour 1", Place = "Place 1", Days = 5, Price = 100, Locations = "Loc 1", TourInfo = "Info 1", IsActive = true, CreatedDate = DateTime.UtcNow, CreatedBy = "Test" },
            new Tour { TourName = "Tour 2", Place = "Place 2", Days = 7, Price = 200, Locations = "Loc 2", TourInfo = "Info 2", IsActive = true, CreatedDate = DateTime.UtcNow, CreatedBy = "Test" },
            new Tour { TourName = "Tour 3", Place = "Place 3", Days = 3, Price = 50, Locations = "Loc 3", TourInfo = "Info 3", IsActive = false, CreatedDate = DateTime.UtcNow, CreatedBy = "Test" }
        );
        await _context.SaveChangesAsync();

        var result = await _repository.GetAllAsync();

        result.Should().HaveCount(2);
        result.All(t => t.IsActive).Should().BeTrue();
    }

    public void Dispose()
    {
        _context.Dispose();
    }
}
